...
def real_challenge(chal):
    global maap
    type = maap[chal][0]
    real_challenge = [maap[chal], maap[chal][1]]  #진정 고려해야할 chal 리스트와 어디까지 고려했는지
    while maap[chal][1] == type:
        if type == 0:
            if real_challenge[2] < maap[chal][2]:
                real_challenge = [maap[chal], chal]
            else:
                real_challenge[1] = chal
        if type == 1:
            if real_challenge[2] > maap[chal][2]:
                real_challenge = [maap[chal], chal]
            else:
...
for i in range(n):  # fly_x == i
    if chal < len(maap) and maap[chal][1] == i:  # 현재 위치에 끈끈이주걱이 있을 때
